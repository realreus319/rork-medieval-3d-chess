...
describe("中国象棋场景坐标", () => {
  it("黑方本营为 rank 1，红方本营为 rank 10", () => {
    expect(positionToSquare({ row: 0, col: 0 })).toBe("a1");
    expect(positionToSquare({ row: 0, col: 8 })).toBe("i1");
    expect(positionToSquare({ row: 9, col: 0 })).toBe("a10");
    expect(positionToSquare({ row: 9, col: 8 })).toBe("i10");
  });
